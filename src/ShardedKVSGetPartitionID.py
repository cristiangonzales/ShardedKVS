...
"""
    Function that will return the paritition ID for the node receiving the HTTP GET request.
    :return HTTP response
"""
class ShardedKVSGetPartitionID(Resource):
    ...
    def get(self):
        try:
            ...
            # Simple flag to break out of nested for loops below
            found = False
            # Partition ID value
            partition_id = None
            # Simply put, all we have to do is iterate through each of the keys in the PartitionDict and find our IP:PORT
            # value amongst the values, and return the key (the key being the partition ID)
            for key, val in partition_dict.items():
                # Iterate through each subarray
                for ip_array in val:
                    # If your local IP:PORT value is in this array, then make the partition_id to be returned to the client
                    # equal to the key (its associated partition_id value
                    if globals.localIPPort in ip_array:
                        partition_id = key
                        # Simple flag to break out of the outer for loop
                        found = True
                        break
                if found is True:
                    break
